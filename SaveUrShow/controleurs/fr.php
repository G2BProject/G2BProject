<?php 

//Header
	$CONNEXION = 'Connexion';
	$DECONNEXION = 'Deconnexion';
	$INSCRIPTION = 'Inscription';
	$TXT_ARTISTES = 'Artistes';
	$TXT_SALLES = 'Salles';
	$TXT_CONCERTS = 'Concerts';
	$TXT_FORUM = 'Forum';
	$RECHERCHE = 'Recherche :';
	$TXT_CHERCHER = 'Chercher';
//Fin header

// Body
	$TXT_SUIVRE = 'Suivez-nous!';
	$TXT_CONNECTES = 'Visites';
	$TXT_INSCRITS = 'Inscrits';
//Fin body


//Footer
	$TXT_FAQ = 'FAQ';
	$TXT_CONTACTS = 'Contacts';
	$TXT_CG = 'Conditions Générales';
//Fin footer


//Connexion (vue)
	//$CONNEXION = 'Connexion'; existe déjà
	$TXT_PSEUDO = 'Pseudo';
	$TXT_MDP = 'Mot de passe';
	$TXT_QUESTION = 'Pas encore inscrit ?';
//Fin Connexion (vue)


//Connexion (controleur)
$ERREUR = 'Une erreur s\'est produite pendant votre identification';
$CHAMPS ='Vous devez remplir tous les champs';
$RETOUR_ACCUEIL = 'pour revenir &agrave la page d\'accueil';
$_ERREUR = 'Le mot de passe ou le pseudo entr&eacute n\'est pas correct';
$RETOUR_PAGE = 'pour revenir &agrave la page pr&eacutec&eacutedente';
// Fin Connexion (controleur)


//Deconnexion (controleur)
$_DECONNEXION = 'Vous êtes déconnecté';
// Fin Deconnexion (controleur)


//Inscription

	$TXT_UTILISATEUR = 'Type d\'utilisateur';
	$TXT_MEMBRE = 'Membre';
	$TXT_ARTISTE = 'Artiste';
	$TXT_PROPRIETAIRE = 'Propri&eacutetaire de salle';
	$TXT_NOM_UTILISATEUR = 'Nom de l\'utilisateur';
	$TXT_MAIL = 'Adresse mail';
	$TXT_CONFIRMER_MAIL = 'Confirmation d\'adresse mail';
	//$TXT_MDP = 'Log in'; existe déjà
	$TXT_CONFIRMER_MDP = 'Confirmation du mot de passe';
	$TXT_NOM = 'Nom';
	$TXT_PRENOM = 'Pr&eacutenom';
	$TXT_SEXE = 'Sexe';
	$TXT_H = 'Homme';
	$TXT_F = 'Femme';
	$TXT_NAISSANCE = 'Date de naissance';
	$JANVIER = 'Janvier';
	$FEVRIER = 'Février';
	$MARS= 'Mars';
	$AVRIL = 'Avril';
	$MAI = 'Mai';
	$JUIN = 'Juin';
	$JUILLET = 'Juillet';
	$AOUT= 'Août';
	$SEPTEMBRE = 'Septembre';
	$OCTOBRE = 'Octobre';
	$NOVEMBRE= 'Novembre';
	$DECEMBRE = 'D&eacutecembre';
	$ADRESSE = 'Adresse';
	$DEPARTEMENT = 'D&eacutepartement';
	$TXT_EX = 'Ex: pour Paris tapez : 75';
	$TXT_PAYS = 'Pays';
	$AVATAR = 'Votre avatar ';
	$FICHIER = 'Choisissez un fichier';
	$TXT_FICHIER = 'Aucun fichier choisi';
	$TXT_NEWS = 'Voulez-vous vous inscrire &agrave la newsletter?';
	$OUI = 'Oui';
	$NON = 'Non';
	$TXT_CGU = 'J\'ai lu et j\'accepte les conditions g&eacuten&eacuterales d\'utilisation du site';
	$TXT_OBLIGATOIRE = 'Champs obligatoires';
	$TXT_S_INSCRIRE = 'S\'inscrire';
//Fin Inscription


//Conditions inscription
$_CONDITION = 'Vous devez accepter les conditions g&eacuten&eacutenerales d\'utilisation du site pour vous inscrire';
$_APPUI = 'Cliquez';
$_ICI = 'ici';
$_REVENIR = 'pour revenir';
$_PSEUDO_UTILISE = 'Le pseudo choisi est d&eacutej&agrave utilis&eacute';
$_CHOIX = 'Vous devez choisir un nom d\'utilisateur';
$_MAIL_UTILISE = 'L\'adresse mail renseign&eacutee est d&eacutej&agrave utilis&eacutee';
$_MEME_MAIL = 'Les deux adresses mails renseign&eacutees doivent &ecirctre identiques';
$_FORMAT_MAIL = 'Votre adresse mail n\'a pas un format valide';
$_MEME_MDP = 'Les deux mots de passe rens&eacuteignes doivent &ecirctre identiques';
$_CHIFFRE = 'Votre mot de passe doit contenir au moins un chiffre';
$_MAJ = 'Votre mot de passe doit contenir au moins une majuscule';
$_MIN = 'Votre mot de passe doit contenir au moins une minuscule';
$_CARACT = 'Votre mot de passe doit comporter au moins 8 caract&egraveres';
$_EXTENSION = 'Extension invalide';
$_TAILLE = 'Le fichier d&eacutepasse la taille limite';
$_DIM = 'L\'image a des dimensions trop importantes';
$_CORRECT = 'Vous &ecirctes correctement inscrit';
//fin Conditions Inscription


//Accueil
$_TOP_ARTISTE = 'Top 3 Artistes';
$_NEW_ARTISTE = 'Nouveaux artistes';
$COEUR = 'Coup de coeur';
$VENIR = 'Concerts &agrave venir';
//Fin Accueil


//Liste artistes
$_RECHERCHE_GENRE = 'Rechercher pas genre';
$POP = 'Pop';
$CLASSIC = 'Classique';
$_MUSIQUE_MONDE = 'Musique du monde';
$METAL = 'Metal';
$REGGAE = 'Reggae';
$VARIETE = 'Vari&eacutet&eacute';
$ALTERNATIF = 'Alternatif';
$ELECT = 'Electro/House';
$_R_B = 'R&ampB';
$JAZZ = 'Jazz';
$RAP = 'Rap';
$ROCK = 'Rock';
// Fin liste artistes


// Artiste (vue)
$QUI = 'Qui';
$STYLE = 'Style de musique';
$A_VENIR = 'A venir';
$PRODUCTION = 'Derni&egraveres productions';
$NBR_FAN = 'Nombre de fans';
$DEVENIR_FAN = 'Devenir fan';
//$BIO = 'Biographie';
$RECEVOIR_ACTU = 'Recevoir l\'actualit&eacute de';
$PAR_MAIL = 'par mail';
//$TXT_NOM_UTILISATEUR = 'Nom de l\'utilisateur';
$DATE = 'Date';
$COMM = 'Laisser un commentaire';
$PUBLIER = 'Publier';
$POST = 'Messages post&eacutes sur cet artiste';
//$TXT_PSEUDO = 'Pseudo';
$DATE_POST = 'Date du post';
// Fin Artiste (vue)


//Contact
$CONTACT = 'Nous contacter';
$NOM = 'Nom, Prénom';
//$TXT_MAIL = 'Adresse mail';
$OBJET = 'Objet';
$MESSAGE = 'Message';
$ENVOI = 'Envoyer';
// Fin Contact


//listeSalles
$SALLES = 'Voici les salles de votre r&eacutegion';
$_SALLES_DEP = 'Si vous voulez voir les salles d\'un autre d&eacute;partement, rentrez le ici';
$DEPARTEMENT = 'D&eacutepartement';
$TXT_EX = 'Ex: pour Paris tapez :75';
$_VOIR_SALLE = 'Voir les salles de ce département';
// Fin listeSalles


//listeConcerts
$CONCERT = 'Voici les concerts de votre d&eacutepartement';
$_CONCERT_DEP = 'Si vous voulez voir les concerts d\'un d&eacutepartement pr&eacutecis, rentrez le ici :';
$DEPARTEMENT = 'D&eacutepartement';
//$TXT_EX = 'Ex: pour Paris tapez :75';
$_VOIR_CONCERT = 'Voir les concerts de ce département';
// Fin listeConcerts


// Profil
$MODIF = 'Modifier';
$DATE_INSCRIT = 'Date d\'inscription';
$BIO = 'Biographie';
$AJOUT_ARTISTE = 'Ajouter un artiste';
$AJOUT_CONCERT = 'Ajouter un concert';
$AJOUT_SALLE = 'Ajouter une salle';
$ADMIN = 'Administration du site';
$NOM_ARTISTE = 'Nom de l\'artiste';
$DATE_CONCERT = 'Date du concert';
$LIEU = 'Lieu';
$NOM_SALLE = 'Nom de la salle';
$PREFERE = 'Artistes pr&eacutef&eacuter&eacutes';
$MES_GENRES = 'Mes genres musicaux';
$GENRE = 'Genre';
// Fin Profil


// Administration/Back Office
//$ADMIN = 'Administration du site';
$REMPLIR = 'Remplissez les champs de texte pour faire les modifications souhait&eacutees';
$VIDE = 'Laissez le champ vide si vous n’y souhaitez pas de modifications';
$SUP_MEMBRE = 'Supprimer le membre suivant';
$SUP_ARTISTE = 'Supprimer l\'artiste suivant';
$SUP_CONCERT = 'Supprimer le concert suivant';
//$SUP_PROPRIO = 'Supprimer le propriétaire de salle suivant';
$CHOIX_NEWS = 'Choisissez ci-dessus les news';
$CHANGER_COEUR = 'Changer le coup de coeur ';
$CHANGER_NEW = 'Choisir le texte de la news' ;
$ACTU_DONNEES = 'Actualisation des données';
$IMAGE_ACTU = "Image de l'actualit&eacute";
$CHANGER_COEURS = 'Changer les coups de coeur' ;
$NUM_1 = 'Num&eacutero 1';
$NUM_2 = 'Num&eacutero 2';
$CHANGER_TOP = 'Changer le top 3';
$TOP_1 = ' Top 1 ';
$TOP_2 = ' Top 2 ';
$TOP_3 = ' Top 3 ';
// Fin Administration/Back Office

//BackOffice (controleur)
$SUPP_COMPTE =' Vous avez supprim&eacute le compte de ';
$PAS_MEMBRE = 'n\'est pas un membre existant';
$SUPP_ARTISTE =' Vous avez supprim&eacute le compte de l\'artiste ';
$PAS_ARTISTE = 'n\'est pas un artiste existant';
$SUPP_CONCERT =' Vous avez supprim&eacute le concert ';
$PAS_CONCERT = 'n\'est pas un concert existant';
$PAS_COEUR = 'Un de vos coups de coeur n\'existe pas';
$MAJ_COEUR = 'Vous aves bien mis à jour vos coups de coeur';
$REMPLIR_COEUR = 'Vous devez remplir le coup de coeur 1 et 2';
$PAS_TOP_ARTISTE = 'Un de vos artistes du top n\'existe pas';
$MAJ_TOP = 'Vous aves bien mis à jour votre top 3';
$CHAMPS_TOP = 'Vous devez remplir tous les champs de votre top';
$MAJ_ACTU = 'Vous avez mis à jour l\'actualité de la page d\'accueil';
// Fin BackOffice (controleur)


// Ajout artiste (vue)
$DESCRIPTION = 'Description';
$IMAGE = 'Image';
// Fin Ajout artiste (vue)

// Ajout artiste (controleur)
$_NOM_UTILISE = 'Le nom choisi est d&eacutej&agrave utilis&eacute';
//$_CHOIX = 'Vous devez choisir un nom d\'utilisateur';
//$_EXTENSION = 'Extension invalide';
//$_TAILLE = 'Le fichier d&eacutepasse la taille limite';
//$_DIM = 'L\'image a des dimensions trop importantes';
// Fin Ajout artiste (controleur)


// Ajout salle (vue)
//$NOM_SALLE = 'Nom de la salle';
//$ADRESSE = 'Adresse';
//$DEPARTEMENT = 'D&eacutepartement';
//$DESCRIPTION = 'Description';
$NUM = 'Num&eacutero de t&eacutel&eacutephone';
$PLACE = 'Nombre de places';
//$IMAGE = 'Image';
//$AJOUT_SALLE = 'Ajouter une salle';
// Fin Ajout salle (vue)


// Ajout salle (controleur)
$_CHOIX_NOM_SALLE = 'Vous devez indiquer le nom de la salle';
$SPECIFIE_DEP_SALLE = 'Vous devez specifier un numéero de département pour votre salle';
$CORRECT_SALLE = 'Votre salle est bien ajoutée';
// Fin Ajout salle (controleur)


// Ajout concert (vue)
$NOM_CONCERT = 'Nom du concert';
//$NOM_SALLE = 'Nom de la salle';
//$NOM_ARTISTE = 'Nom de l\'artiste';
$AFFICHE = 'Affiche du concert';
//$DATE_CONCERT = 'Date du concert';
$HEURE_CONCERT = 'Heure du concert';
// Fin Ajout concert (vue)


// Ajout concert (controleur)
$_CHOIX_NOM_CONCERT = 'Vous devez indiquer le nom du concert';
//$_CHOIX_NOM_SALLE = 'Vous devez indiquer le nom de la salle';
$_CHOIX_NOM_ARTISTE = 'Vous devez indiquer le nom de l\'artiste';
$PAS_NOM = 'Le nom de l\'artiste choisi n\'existe pas';
//$_CHOIX = 'Vous devez choisir un nom d\'utilisateur';
$CORRECT_CONCERT = 'Votre concert a bien été ajoutée';
// Fin Ajout concert (controleur)


// Concert (vue)
$DATE = 'Date';
$LIEU = 'Lieu';
$OU = 'O&ugrave r&eacuteserver';
$PLACE = 'Nombre de places';
$LOCAL_CONCERT = 'Localiser le concert';
//$RECEVOIR_ACTU = 'Recevoir l\'actualit&eacute de ';
//$PAR_MAIL = 'par mail';
$DESCRIPTION_CONCERT = 'Description du concert';
//$TXT_NOM_UTILISATEUR = 'Nom de l\'utilisateur';
//$DATE = 'Date';
//$COMM = 'Laisser un commentaire';
//$PUBLIER = 'Publier';
$POST_CONCERT = 'Messages post&eacutes sur ce concert';
//$TXT_PSEUDO = 'Pseudo';
//$DATE_POST = 'Date du post';
// Fin Concert (vue)

//listeConcerts (vue)
//$NOM_CONCERT = 'Nom du concert';
//$DATE = 'Date';
$ET = ' et ';
//$HEURE_CONCERT = 'Heure du concert';
//$NOM_SALLE = 'Nom de la salle';
//Fin Concerts (vue)


// listeSalle (vue)
//$NOM_SALLE = 'Nom de la salle';
//$ADRESSE = 'Adresse';
$NUM_SALLE = 'Num&eacutero';
// Fin listeSalle (vue)

//Forum (vue)
$CREER_DISCUSSION = 'Créer discussion';
$CONNECT = 'Veuillez vous connecter pour créer un post';
$INSCRIRE = 'inscrivez-vous';
$PROPOS = 'A PROPOS DU SITE';
$SUJET = 'SUJETS/MESSAGES';
$AIDE = 'Aide,Bugs du site';
$ACTUALITE = 'Actualités du site, aide et questions diverses,suggestions';
$PBM = 'Posez toutes vos questions en cas de problèmes, et faites nous part de vos remarques,souhaits d\'amélioration,bugs';
$AVIS = 'Avis et Suggestions';
$PART_AVIS = 'Vous pouvez nous faire part de vos avis afin d\'améliorer le site';
$DISCUSSION = 'DISCUSSIONS';
$GROUPES = 'Groupes/Artistes';
$DISCUTE_GROUPE = 'Venez discuter de vos groupes/artistes préférées';
//$TXT_CONCERTS = 'Concerts';
$DISCUTE_DATE = 'Discuter des prochaines dates de concerts';
// Fin Forum (vue)


//Sous-catégorie 1, 2, 3 et 4
$_AIDE = 'AIDE,ACTUALITES DU SITE';
$NUM_MESSAGE = 'NOMBRE DE MESSAGES';
$MAJ_CONCERT = 'CONCERTS';
// Fin Sous-catégorie 1, 2, 3 et 4


//Discussion (vue)
$CATEGORIE = 'Catégories';
$SUJET_DISCUSSION = 'Sujets';
$VALIDE = 'Valider';
//Fin Discussion (vue)
?>
